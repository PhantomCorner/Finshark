...
export const getCashFlow = async function (query: string) {
  try {
    const res = await axios.get<CompanyCashFlow[]>(
      `https://financialmodelingprep.com/api/v3/cash-flow-statement/${query}?limit=40&?&apikey=${VITE_API_KEY}`
    );
    return res;
  } catch (e: any) {
    console.log("error message from API", e);
...
export const getTenK = async function (query: string) {
  try {
    const res = await axios.get<CompanyTenK[]>(
      `https://financialmodelingprep.com/api/v3/sec_filings/${query}?type=10-k&page=0&apikey=${VITE_API_KEY}`
    );
    return res;
  } catch (e: any) {
    console.log("error message from API", e);
